...
namespace falleng {
    ...
    private:
        ...
        void analize_hand(std::array<bool, 12>& allowed_combos);
        bool is_allowed(std::array<bool, 12>& allowed_combos, Combo combo);
    ...
    template<class Card_type>
    template<class FWIterator>
    void Hand<Card_type>::set_cards(std::array<bool, 12>& allowed_combos, FWIterator begin, FWIterator end)
    {
        for (;begin != end; begin++){
            m_cards.push_back(*begin);
        }
        analize_hand(allowed_combos);
    }

    template<class Card_type>
    bool Hand<Card_type>::is_allowed(std::array<bool, 12>& allowed_combos, Combo combo)
    {
        return allowed_combos[static_cast<int>(combo) - 1];
    }
    ...
    template<class Card_type>
    void Hand<Card_type>::analize_hand(std::array<bool, 12>& allowed_combos)
    {
        ...
        if (is_allowed(allowed_combos, Combo::Casa_grande) && card_1 == 1 && card_2 == 12 && card_3 == 12){
            m_combo = Combo::Casa_grande;
            m_combo_name = "Casa Grande";
            return;
        }
        // Check for Casa Chica (1, 11, 11)
        else if (is_allowed(allowed_combos, Combo::Casa_chica) && card_1 == 1 && card_2 == 11 && card_3 == 11){
            m_combo = Combo::Casa_chica;
            m_combo_name = "Casa Chica";
            return;
        }
        // Check for Registrico (1, 10, 11)
        else if (is_allowed(allowed_combos, Combo::Registrico) && card_1 == 1 && card_2 == 10 && card_3 == 11){
            m_combo = Combo::Registrico;
            m_combo_name = "Registrico";
            return;
        }
        // Check for Maguaro (1, 10, 12)
        else if (is_allowed(allowed_combos, Combo::Maguaro) && card_1 == 1 && card_2 == 10 && card_3 == 12){
            m_combo = Combo::Maguaro;
            m_combo_name = "Maguaro";
            return;
        }
        // Check for Registro (1, 11, 12)
        else if (is_allowed(allowed_combos, Combo::Registro) && card_1 == 1 && card_2 == 11 && card_3 == 12){
            m_combo = Combo::Registro;
            m_combo_name = "Registro";
            return;
        }
        // Check for Vigia ( X, X, {X+1}) or ( X, {X+1}, {X+1})
        else if (is_allowed(allowed_combos, Combo::Vigia) && (  (card_1 == card_2 && card_3 == card_1 + 1)
        ...
        else if (is_allowed(allowed_combos, Combo::Patrulla) && (card_1 + 1 == card_2 && card_1 + 2 == card_3))
        {
        ...
        else if (is_allowed(allowed_combos, Combo::Trivilin) && card_1 == card_2 && card_2 == card_3)
        {
        ...
        // Check for Ronda... ( X, X, *) or ( *, X, X)
            ...
            if (is_allowed(allowed_combos, Combo::Ronda_12) && card_2.value() == 12){
                // Ronda_12 (pair of 12)
                m_combo = Combo::Ronda_12;
            }
            else if (is_allowed(allowed_combos, Combo::Ronda_11) && card_2.value() == 11){
                // Ronda_11 (pair of 11)
                m_combo = Combo::Ronda_11;
            }
            else if (is_allowed(allowed_combos, Combo::Ronda_10) && card_2.value() == 10){
                // Ronda_10 (pair of 10)
                m_combo = Combo::Ronda_10;
            }
            else if (is_allowed(allowed_combos, Combo::Ronda)){
                // Ronda (pair of [1-7])
                m_combo = Combo::Ronda;
            }
            m_combo_name = "Ronda";
